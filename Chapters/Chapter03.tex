...
In these cases, cross-validation is used to obtain more accurate performance measures. Cross-validation performs a division of the data into several parts or ``folds'', and iteratively use some of these fold for training or testing. The simplest form of cross-validation is $K$-fold, in which the data is divided in $K$ subsets, reserving one for testing and training the model using the rest. 


All performance results shown here have been obtained using a 10-Fold cross-validation, as suggested in \cite{Kohavi1995}, reducing the variance of other widely known forms of cross-validation like leave-one-out. 
In contrast to these, 


This procedure is repeated $K$ times, and a confusion matrix is obtained. 
...
\begin{table}
	...
	\begin{tabular}{cc|c|c|}
		...
		& &  Positive & Negative \\ 
		\hline
		\multirow{2}{*}{Actual}& Positive & True Positive & False Positive \\ 
		\cline{2-4} 
...
Table~\ref{tab:confmat} can be easily extended to multiclass, by adding rows and columns, and substituting the ``positive'' and ``negative'' by the class number. The confusion matrix is the basis for computing other performance measures, such as accuracy, sensitivity or specificity.



 the strategy to obtain accurate performance measures is by cross-validation \cite{Kohavi1995} 
in neuroimaging, sample size is an issue. 
In fields where the sample size is an issue, like neuroimaging, 
