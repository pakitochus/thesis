...
\end{equation}
with $\mathbf{p}_0$ being the origin of the spherical coordinates, and $\mathbf{p}_N$ is the last possible coordinate within $\mathbb{I}$ in the current direction $(\varphi,\theta)$. In this work, we have placed $\mathbf{p}_0$ at the \ac{AC} of the image, although other options, such as setting the origin at the middle point of $\mathbb{I}$ could be considered. This choice is a convention when using the \ac{MNI} coordinates \cite{Evans1993}, sharing conectivity with both hemispheres, therefore allowing the optimal computation of our paths. 

% DONE


where $\mathbf{p}_0$ is the node located at the origin of the image and $\mathbf{p}_N$ is the theoretical projection of the current direction $(\varphi,\theta)$ in the limits of the image $\mathbb{I}$. Setting $\mathbf{x}_0$ at the AC is not a random choice; it is a convention when using the \ac{MNI} coordinates\cite{Evans1993} and furthermore, since it is a point that shares connectivity with both hemispheres, the resulting paths will be optimal, covering most of the brain. We can assume a first-order \acf{HMM} for the tracing of the path, and the $i$-th node will be computed as:
\begin{equation}
...
\begin{equation}
P(\mathbb{P}_{\varphi,\theta}) = P(\mathbf{x}_0, \mathbf{x}_1, \dots \mathbf{x}_N) = \prod_{i=1}^{N} P(\mathbf{x}_i|\mathbf{x}_{i-1})
\end{equation} 
...
P(\mathbb{P}_{\varphi,\theta}|\mathbf{I}) & = P(\mathbf{x}_0, \dots x_N| I(\mathbf{x}_0), \dots I(\mathbf{x}_N))\\
...
For computational purposes, we will compute all the needed probabilities on a set of candidates $\mathbb{P}_{\varphi,\theta}_c = \{\mathbf{x}_{c,1}, \mathbf{x}_{c,2}, \dots \mathbf{x}_{c,M} \}$ defined by $\mathbf{x}_{i-1}$. This candidates are contained inside the $L^2$-norm support ball $B_{2,r}(\mathbf{x}-\mathbf{x}_{i-1})$ of radius $r$ centred in $\mathbf{x}_{i-1}$.
...
\begin{algorithm*}
	...
	\While{$(i<\text{IterLimit})$ \& $(I(\mathbf{x}_i)>I_{th})$ \& ($\mathbf{x}_i \in \mathbb{I}$)}{
		Get the node candidates $\mathbb{P}_{\varphi,\theta}_c = \{\mathbf{x}_{c,1}, \mathbf{x}_{c,2}, \dots \mathbf{x}_{c,M}\}$ where $\mathbf{x}_{c,m} \in B_{2,r}(\mathbf{x}_{c,m}-\mathbf{x}_i)$\;
		Get the intensities of the candidates $I(\mathbf{x}_c) \quad \forall \mathbf{x}_c \in \mathbb{P}_{\varphi,\theta}_c$\;
		\textbf{foreach} $\mathbf{x}_c \in \mathbb{P}_{\varphi,\theta}_c${
	...
	$\mathbb{P}_{\varphi,\theta}_{opt}\leftarrow \{\mathbf{x}_0, \mathbf{x}_1, \dots \mathbf{x}_N\}$\; 
	%	}
	%}
...
Our modification will compute a node-wise GLC matrix, in which the number of grey-level transitions between adjacent nodes, noted as $\mathbf{x}_i$ and $\mathbf{x}_{i+1}$, is stored along the whole path $\mathbb{P}_{\varphi,\theta} = \{\mathbf{x}_0, \mathbf{x}_1 \dots \mathbf{x}_N\}$. Mathematically, the computation of the GLCM in each point in the path will be: 
