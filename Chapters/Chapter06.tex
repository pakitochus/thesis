...
Individual probabilities $P (I(\mathbf{p}_i)|\mathbf{p}_i)$ needed in the computation of Eq.~\ref{eq:intP} can be computed under the assumption of a normally distributed candidate set $V_{\varphi,\theta}^c$ with mean $I(\mathbf{p}_{i-1})$ and variance $\sigma_c^2$. We will estimate the probability of the i$^th$ candidate node $\mathbf{p}_i$ as: 
...
\begin{algorithm*}
	...
	\Output{List of nodes in the optimum path $\mathbb{P}_{\varphi,\theta}_{opt}$}
	\BlankLine
	...
	\While{$(i<\text{IterLimit})$ \& $(I(\mathbf{p}_i)>I_{th})$ \& ($\mathbf{p}_i \in \mathbb{I}$)}{
		Get the node candidates $\mathbb{P}_{\varphi,\theta}_c = \{\mathbf{p}_{c,1}, \mathbf{p}_{c,2}, \dots \mathbf{p}_{c,M}\}$ where $\mathbf{p}_{c,m} \in B_{2,r}(\mathbf{p}_{c,m}-\mathbf{p}_i)$\;
		Get the intensities of the candidates $I(\mathbf{p}_c) \quad \forall \mathbf{p}_c \in \mathbb{P}_{\varphi,\theta}_c$\;
		\textbf{foreach} $\mathbf{p}_c \in \mathbb{P}_{\varphi,\theta}_c${
	...
	$\mathbb{P}_{\varphi,\theta}_{opt}\leftarrow \{\mathbf{p}_0, \mathbf{p}_1, \dots \mathbf{p}_N\}$\; 
	%	}
	%}
