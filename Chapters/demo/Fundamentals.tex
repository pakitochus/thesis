...
Una formula ``in linea'' è incorporata nel testo: $\lim_{n \to \infty}\sum_{k=1}^n \frac{1}{k^2} = \frac{\pi^2}{6}$, per esempio. Come si può osservare, \LaTeX{} fa \emph{il possibile} per comprimerla e modificare meno che può l'interlinea nel capoverso che la contiene.
Una formula in display è un'espressione che \LaTeX{} compone su linee a sé, separate dal contesto con adeguati spazi bianchi per ``metterla in mostra'' e farla risaltare sulla pagina.
\begin{equation}
