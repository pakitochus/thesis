...
\acf{FA} was used in \cite{Martinez201141,Martinez-Murcia20129676} to perform feature extraction in \ac{CAD} systems. This strategy assumes that each image in the database is a realization of a given experiment. \ac{FA} then models each of the $N$ observations (or subjects) as the expression of $c$ unobserved variables, known as factors. The model follows Eq. \ref{eq:FactorAn1}.

\begin{equation}\label{eq:FactorAn1}
\mathbf{x}_i = \mu + \boldsymbol{\lambda}_i \mathbf{F} + \epsilon
\end{equation}
where $\mathbf{x}_i$ is the $i$-th realization of the experiment, $\mu$ is the mean of $\mathbf{x}_i$,  $\boldsymbol{\lambda}_i$ is the representation of $\mathbf{x}_i$ in the space defined by the factors $\mathbf{F}$, and $\epsilon$ is the reconstruction error. For all observations, Eq. \ref{eq:FactorAn1} can be rewritten using matrix notation as as:
